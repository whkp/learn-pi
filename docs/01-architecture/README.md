...
这个式子给出的是**核心循环的最小闭环**：让循环转起来，缺一个机制都不成立，所以只有两项。它不试图穷尽一个 Agent 系统的全部组成——经典文献里的 Planning 与 Memory 不在式子中，但在 Pi 里都有明确的承担者。

### 与经典组件图的对应关系

经典文献通常把 Agent 写成四个组件：**Agent = LLM + Planning + Memory + Tools**。这个拆法回答"**一个 Agent 系统由什么组成**"；`agent = LLM + tool use` 回答"**让循环转起来，最少需要哪些机制**"。两者层次不同，并不矛盾，两个在式子里缺席的组件各有承担者：
...
**一句话总结**：这个式子没有漏掉 Planning 和 Memory——Planning 由模型在循环内每步"选哪个工具"时完成，Memory 拆成消息数组、会话、压缩三件机制，由产品层各自承担。

## 二、Pi 的三层架构

既然能力边界由 harness 决定，它内部又如何组织？Pi 的分层可以概括为三句话：
...
Pi 刻意不内置子代理、计划模式、权限弹窗、MCP——每个团队的工作流不同，内置会限制灵活性。更根本的原因在于：**这些能力本质上都是一个工具**。
...
- 核心包仍为 `ai`、`agent`、`tui`、`coding-agent` 四个；实验性包 0.85 新增 `chord`（与具体应用无关的基础设施：插件加载与组合、服务声明、对称 RPC、状态复制；官方标注尚非稳定 API）。
...
- 分层是设计边界，不是安全边界：权限、认证、沙箱必须在产品层实现，核心不做兜底。
